...
object KPID5_2 {
  def main(args: Array[String]){
    ...
    val res = data.where("_c7=\"M\"").groupBy("_c3").agg(sum("_c8").alias("a_sum")).orderBy(desc("a_sum")).limit(3)
    
    val res1 = data.join(res,"_c3").where("_c7=\"F\"").groupBy("_c3", "_c4").agg(sum("_c9").alias("r_sum"))
    ...
    res2.rdd.coalesce(1, true)saveAsTextFile("/home/hduser/eclipse-workspace/AadharProject/kp52outDS")
