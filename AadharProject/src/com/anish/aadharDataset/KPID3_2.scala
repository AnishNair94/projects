...
object KPID3_2 {
  def main(args: Array[String]){
    ...
    val fresult = data.groupBy("_c2").agg(sum("_c9").alias("asum")).orderBy(desc("asum")).select("_c2").limit(3)
    
    fresult.rdd.saveAsTextFile("/home/hduser/eclipse-workspace/AadharProject/kp32outDS")
    spark.stop()
